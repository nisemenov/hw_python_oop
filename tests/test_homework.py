...
class TestRecord:
    ...
    def test_init(self, kwargs, msg_err):
        ...
        if 'date' in kwargs:
            assert result.date == datetime.strptime(kwargs['date'], '%d.%m.%Y').date(),                 msg_err('wrong_attr', 'date', 'Record', ', свойство должно быть датой')
        else:
            assert result.date == datetime.now().date(),                 msg_err('wrong_attr', 'date', 'Record', ', свойство должно быть датой')
...
class TestCalculator:
    ...
    def test_get_calories_remained(self, init_limit, msg_err):
        result = homework.Calculator(init_limit)
        assert not hasattr(result, 'get_calories_remained'),             msg_err('dont_create_method', 'get_calories_remained', 'Calculator')

    def test_get_today_cash_remained(self, init_limit, msg_err):
        result = homework.Calculator(init_limit)
        assert not hasattr(result, 'get_today_cash_remained'),             msg_err('dont_create_method', 'get_today_cash_remained', 'Calculator')


class TestCaloriesCalculator:

    def test_init(self, init_limit, msg_err):
        assert hasattr(homework, 'CaloriesCalculator'),             msg_err('add_class', 'CaloriesCalculator', child=True, parent_name='Calculator')
    ...
    def test_get_calories_remained(self, init_limit, data_records,
        ...
        result = homework.CaloriesCalculator(init_limit)
        assert hasattr(result, 'get_calories_remained'),             msg_err('add_method', 'get_calories_remained', 'CaloriesCalculator')
        ...
        if today < init_limit:
            assert result.get_calories_remained() == positive_calories_remained(init_limit - today),                 msg_err('wrong_method', 'get_calories_remained', 'CaloriesCalculator')
            result.limit = today - 200
            assert result.get_calories_remained() == negative_calories_remained,                 msg_err('wrong_method', 'get_calories_remained', 'CaloriesCalculator')
        else:
            assert result.get_calories_remained() == negative_calories_remained,                 msg_err('wrong_method', 'get_calories_remained', 'CaloriesCalculator')
            result.limit = today + 200
            assert result.get_calories_remained() == positive_calories_remained(init_limit - today),                 msg_err('wrong_method', 'get_calories_remained', 'CaloriesCalculator')
...
class TestCashCalculator:
    ...
    def test_init(self, init_limit, msg_err):
        assert hasattr(homework, 'CashCalculator'),             msg_err('add_class', 'CashCalculator', child=True, parent_name='Calculator')
    ...
    def test_get_today_cash_remained(self, init_limit, data_records, amount, currency, today_cash_remained, msg_err,
        ...
        result = homework.CashCalculator(init_limit)
        assert hasattr(result, 'get_today_cash_remained'),             msg_err('add_method', 'get_today_cash_remained', 'CashCalculator')
        ...
        assert re.fullmatch(today_cash_remained(amount, currency), result.get_today_cash_remained(currency)),             msg_err('wrong_method', 'get_today_cash_remained', 'CashCalculator')
